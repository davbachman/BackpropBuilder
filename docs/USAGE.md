...
Choose **Custom CSV…** from that dropdown to open a local file picker. Each output is labeled with its CSV column name. Wire the desired column to a **Target** block; that connection designates the target, and all other columns become numeric features. Before you make that connection, the last column is used for preview (or the sole text-label column, if there is one). The Dataset panel lets you check header detection, choose regression or classification, and replace the file. Imported rows remain in the saved project JSON, so the original CSV file is not needed when reopening it.
