...
export default function LearningStudio() {
  const [lesson, setLesson] = useState<LessonKind | 'blank' | null>(null)
  if (lesson)
    return (
      <App
        key={lesson}
        initialGraph={createModelPreset(lesson)}
        presetKind={lesson}
        onGallery={() => setLesson(null)}
      />
    )
  return (
    <div className="studio studio-gallery">
      <header className="studio-header">
        ...
        <div className="studio-header-actions">
          <button onClick={() => setLesson('blank')}>
        ...
        <section className="gallery-hero">
          <div>
            ...
            <p className="hero-copy">
              Start with a single weight. Follow the numbers through a network.
              Zoom into a transformer and see where its next token comes from.
            </p>
            <div className="hero-meta">
              <span>
                <Layers3 size={16} /> {LESSONS.length} editable model presets
              </span>
            ...
            <svg viewBox="0 0 380 210">
              <g fill="none" stroke="#d8cff0" strokeWidth="2">
                ...
                {[40, 85, 130, 175].map((y, i) => (
                  <path key={i} d={`M 206 ${y} C 268 ${y}, 268 105, 330 105`} />
                ))}
              </g>
              {[55, 105, 155].map((y, i) => (
                <g key={i}>
